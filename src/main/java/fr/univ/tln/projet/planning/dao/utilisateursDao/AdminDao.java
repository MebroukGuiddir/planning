...
import fr.univ.tln.projet.planning.modele.utilisateurs.Admin;
...
public class AdminDao extends UtilisateurDao <Admin> {
    ...
    { List<Utilisateur> listAdmin=new ArrayList();
        ...
        try (Connection connection = this.getConnection();
             ...
             PreparedStatement statement =
                     connection.prepareStatement("SELECT * FROM admin a   JOIN utilisateur u ON a.id_user=u.id_user WHERE ( nom LIKE ?   OR  prenom LIKE ?  ) ")){
    ...
    public   Admin trouver(String username,String password ) throws DaoException{
        try (Connection connection = this.getConnection();
            ...
            if (!rs.next( ))
                throw new ObjetInconnuDaoException("Admin inexistante id_user: "+username);
